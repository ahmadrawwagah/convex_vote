...
__export(typecheck_exports, {
  processTypeCheckResult: () => processTypeCheckResult,
...
async function processTypeCheckResult(ctx, typeCheckMode, runTypeCheck) {
  if (typeCheckMode === "disable") {
    return;
  }
  const result = await runTypeCheck();
  if (result === "cantTypeCheck" && typeCheckMode === "try") {
    console.error(
      import_chalk.default.gray("To always skip typechecking, use `--typecheck=disable`.")
    );
  }
...
async function typeCheckFunctions(ctx, functionsDir) {
...
async function runTsc(ctx, tscArgs) {
  const tscPath = import_path.default.join(
    "node_modules",
    ".bin",
  ...
  const versionResult = import_child_process.default.spawnSync(tscPath, ["--version"]);
  if (!ctx.fs.exists(tscPath)) {
    console.error(
      import_chalk.default.gray("No TypeScript binary found, so skipping typecheck.")
    );
  ...
  const typeCheckResult = await runTscInner(ctx, tscPath, tscArgs);
...
async function runTscInner(ctx, tscPath, tscArgs) {
  const result = import_child_process.default.spawnSync(
    tscPath,
    tscArgs.concat("--listFiles")
  );
  if (result.status === null) {
    console.error(import_chalk.default.red(`TypeScript typecheck timed out.`));
    if (result.error) {
      console.error(import_chalk.default.red(`${result.error}`));
    }
  ...
  try {
    import_child_process.default.execFileSync(
      tscPath,
      tscArgs,
      { stdio: "inherit" }
    );
    ...
    console.error(import_chalk.default.red("TypeScript typecheck via `tsc` failed."));
    return "typecheckFailed";
  }
}
