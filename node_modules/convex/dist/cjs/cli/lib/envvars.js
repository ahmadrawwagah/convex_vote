...
async function offerToWriteToEnv(ctx, type, value, saveUrl = "ask") {
  ...
  if (detectedFramework) {
    console.error(import_chalk.default.green(`Detected ${detectedFramework} project.`));
  }
  ...
  if (existing) {
    ...
    if (matching.length === 1) {
      ...
      if (oldValue === value) {
        console.error(
          import_chalk.default.green(`${envVar2} in ${envFile} is already ${value}`)
        );
        return;
      }
      ...
      if (saveUrl === "yes" || (await import_inquirer.default.prompt([
        {
          type: "confirm",
          name: "updateEnvFile",
          message: `Update ${envVar2} in ${envFile} to ${value} to save ${type} URL?`,
          default: true
        }
      ])).updateEnvFile) {
      ...
      console.error(
        import_chalk.default.yellow(`Please update ${envFile} with ${envVar2}="${value}"`)
      );
      return;
    }
    if (saveUrl === "yes" || (await import_inquirer.default.prompt([
...
`;
      ctx.fs.writeUtf8File(envFile, modified);
      console.error(
        import_chalk.default.green(`Updated ${envFile} with ${envVar}="${value}"`)
      );
    }
    return;
  }
