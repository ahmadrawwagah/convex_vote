"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var storage_impl_exports = {};
__export(storage_impl_exports, {
  setupStorageHttpWriter: () => setupStorageHttpWriter,
  setupStorageReader: () => setupStorageReader,
  setupStorageWriter: () => setupStorageWriter
});
module.exports = __toCommonJS(storage_impl_exports);
var import_syscall = require("./syscall.js");
var import_validate = require("./validate.js");
function setupStorageReader() {
  return {
    getUrl: async (storageId) => {
      (0, import_validate.validateArg)(storageId, 1, "getUrl", "storageId");
      return await (0, import_syscall.performAsyncSyscall)("storageGetUrl", { storageId });
    },
    getMetadata: async (storageId) => {
      return await (0, import_syscall.performAsyncSyscall)("storageGetMetadata", { storageId });
    }
  };
}
function setupStorageWriter() {
  const reader = setupStorageReader();
  return {
    generateUploadUrl: async () => {
      return await (0, import_syscall.performAsyncSyscall)("storageGenerateUploadUrl", {});
    },
    delete: async (storageId) => {
      await (0, import_syscall.performAsyncSyscall)("storageDelete", { storageId });
    },
    getUrl: reader.getUrl,
    getMetadata: reader.getMetadata
  };
}
function setupStorageHttpWriter() {
  const writer = setupStorageWriter();
  return {
    ...writer,
    store: async (request) => {
      return await (0, import_syscall.performJsSyscall)("storage/storeFile", {
        request
      });
    },
    get: async (storageId) => {
      return await (0, import_syscall.performJsSyscall)("storage/getFile", {
        storageId
      });
    }
  };
}
//# sourceMappingURL=storage_impl.js.map
